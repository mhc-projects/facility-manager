...
- `dpf_service_record_attachments` — 접수이력 첨부파일 12고정슬롯 (record_id FK cascade, slot_key: 12개 CHECK 제약, storage_path, uploaded_by FK employees). `dpf_service_records`의 소프트 삭제와 별개로 하드 삭제. 비공개 버킷(`dpf-attachments`) + `createSignedUrl` — 상세는 [[dpf]] 스킬 참고.
