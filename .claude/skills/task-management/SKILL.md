...
- ⚠️ 보조금 계열 3개 카테고리(보조금·보조금(5년경과)·보조금 동시진행)는 같은 라벨에 다른 stage_key를 쓰는 단계가 있다(예: "[보조금]승인(착공신청서제출필요)"가 보조금은 `custom_1777968825327`, 5년경과는 `custom_1778198486933`; "설치완료(준공도서 작성 필요)"가 보조금은 `subsidy_pre_completion_document_submit`, 5년경과는 `subsidy_installation`). 반대로 같은 키가 카테고리마다 다른 라벨인 경우도 있다(`subsidy_installation`은 동시진행에서 "설치완료"). 사업장 진행구분이 바뀌거나(재분류) 예전 단계 목록으로 옮긴 업무는 status 키가 자기 카테고리에 없는 "고아"가 되는데, 2026-09-09부터 표시 계층은 `resolveStageKey(status, progressStatus)`(`contexts/AdminDataContext.tsx`)가 그 키의 라벨로 같은 라벨의 카테고리 키를 찾아 배치한다(칸반·목록·수정 모달·미니칸반). 업무 `status` 원본은 건드리지 않고 다음 드래그가 올바른 키를 쓴다. 단계 추가 API(`POST /api/settings/task-stages`)는 키 미지정 시 같은 task_type 카테고리의 같은 라벨 키를 재사용해 새 분기를 막는다. 복수 진행구분 칸반은 칸을 라벨로 합치므로 grouped 키도 라벨이다(status 키로 묶으면 같은 키·다른 라벨 칸끼리 덮어씀).
