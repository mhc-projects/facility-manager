...
## 핵심 DB 테이블
- `business_sites` — 사업장 정보 + 마감 상태 (closing_status, forecast_closing_date 등)
- `financial_records` — 매출/세금계산서/입금 관리
- `equipment_inventory` — 센서/게이트웨이/장비 수량
- `inspections` — 실사 관리 (견적/착공/준공)
- `subsidies` — 보조금 관리
- `users` — 계정 (권한레벨 1:일반, 2:팀장, 3:관리자)
- `work_tasks` — 업무 게시판 (지시/수행/이관/협조)
