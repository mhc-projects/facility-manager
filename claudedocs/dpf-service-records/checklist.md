...
## 최종 재점검 A/B/C 구현 (2026-09-13, 계속)
사용자 요청: "크린어스 시스템에서 사용되고 있는 항목이 우리 시스템에 빠진게 없는지 확인해줘. 어드바이저에게 검토
받아줘." → 필드 단위 재감사로 A(폼엔 있고 DB에도 저장되지만 어떤 화면에도 안 보이는 필드)/B(구변일·부착장치
목록 컬럼 부재)/C(기간 프리셋 부재) 발견 → 사용자 지시 "A, B, C 다 고쳐줘" → 구현.

- [x] A — `app/dpf/[vin]/page.tsx` 접수이력 카드: 처리기사/비용/필터/회수필터/교체필터/택배사/접수유형/배송주소를
      `dl`에 추가, 세부내용/처리내용/연장사유/비고를 라벨 붙은 문단으로 추가, 긴급/통화요청/출동/입고를
      `bg-slate-100` 배지로 헤더에 추가(상태·연장·전환 배지와 축이 다름을 색으로 구분).
      `DpfServiceRecordTable.tsx`의 `billing_status` 셀에 `cost_type`도 3번째 줄로 폴드(양쪽 variant 공통 —
      폼에서 비용 섹션이 카테고리 무관 항상 노출됨을 확인 후 반영, 처음엔 "물류 전용"으로 오판할 뻔했으나
      어드바이저가 폼을 먼저 확인하라고 지적).
- [x] B — `types/dpf.ts`의 `DpfServiceRecordWithVehicle` Pick에 `installation_date`/`device_type` 추가,
      `app/api/dpf/service-records/route.ts` 조인 확장. 새 컬럼 대신 기존 셀에 폴드: `vin` 셀에 구변일
      2번째 줄, `vehicle_name` 셀에 부착장치 배지(색상 매핑 `DEVICE_TYPE_COLORS`를 `DpfVehicleTable.tsx`에서
      모듈 export로 빼내 재사용 — 색상표 두 곳에 중복 금지).
- [x] C — `DpfServiceListView.tsx`에 1/3/6/12개월 프리셋 버튼 추가. `dateFrom`만 설정하고 `dateTo`는 비움(열린
      범위) — 닫힌 범위로 오늘까지 잡으면 `date_field`가 미래 예약 가능한 필드(접수일 등)일 때 당일 이후 값이
      빠지는 경계 문제가 생긴다는 어드바이저 지적 반영. 로컬 날짜 기준 `monthsAgoLocal()` 헬퍼로 계산(UTC
      `toISOString()` 쓰면 KST 09:00 이전엔 하루 밀림 — 이번 세션 `created_at` 버그와 동일 계열이라 재발 방지).
      말일 오버플로(예: 05-31 - 3개월)는 `setDate(0)`으로 클램프. design doc §8.1의 "기간 프리셋은 `/dpf` 패턴
      재사용" 문장이 틀린 전제였음을 정정(그 전제인 `/dpf` 자체엔 프리셋이 없음, bucket (d) 확인 사항).
- [x] 물류 레코드 1건으로 A/B/C 동시 검증(85가8787, 부품전달, 접수일 2026-07-13 = 오늘 기준 정확히 2개월 전,
      처리기사/비용=유상/필터×3/택배사/배송주소/접수유형=요청/긴급+출동/세부내용/처리내용/비고/연장요청+사유
      전부 채움) → 카드에 전체 필드 순서대로 렌더 확인 → 물류관리 목록에 비용(청구상태 셀)/부착장치(차명
      셀)/구변일(차대번호 셀) 폴드 확인 → 1개월 프리셋 0건, 3개월 프리셋 1건 확인 → 필터 배지 카운트 증가 확인 →
      접수현황 하드리로드로 스켈레톤 헤더 12개 확인(로딩 중이라 실데이터 건수는 미확인 — reception "전체"는
      category 필터를 안 보내 물류 테스트 레코드도 포함돼 이 시점엔 1건이었을 것) + 콘솔 에러 없음 확인 →
      `DpfVehicleTable.tsx`(DEVICE_TYPE_COLORS 추출로 변경됨)도 `/dpf` 하드리로드로 장치 컬럼 배지 정상·콘솔
      클린 확인 → 소프트 삭제, DB 하드 삭제 SQL 전달
...
## 슈퍼관리자 접수 영구 삭제 (2026-09-23)
배경: 운영 테스트 접수를 UI로 지워도(소프트 삭제) round_no 채번에 계속 잡혀 SQL로 하드 삭제해야 했음 → 사용자 요청
"권한 4는 UI에서 할 수 있으면 좋겠다". 방식은 사용자 선택("삭제된 접수 보기 + 영구 삭제", 복구 버튼은 제외).
- [x] `GET /api/dpf/vehicles/[vin]?include_deleted=1` — 권한 4일 때만 `deletedServiceRecords` 별도 필드 반환
