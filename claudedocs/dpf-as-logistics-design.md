...
→ 2026-09-13: 위 문단의 "목록/카드 모두 안 만든다"는 §6-3(미관찰) 근거였는데, 이번 건은 크린어스 관찰과 무관하게
**사용자가 직접 요청**한 것이라 그 근거가 적용되지 않는다. `/dpf/[vin]` 접수이력 **카드**에 첨부 개수 배지(📎)를
추가(`GET /api/dpf/vehicles/[vin]`가 배치 쿼리로 개수만 반환, 서명 URL 발급 없음). **목록**(`DpfServiceRecordTable`)은
사용자 결정으로 보류 — 카드로 써보고 불편하면 목록까지 진행하기로 함(`checklist.md`/`.claude/skills/dpf/SKILL.md` 참고).
...
- 19컬럼에 6개를 더하지 않고 **4개(최근접수일/마지막처리일/처리횟수/구조변경경과일, 단일 라인) + 액션버튼 1개 = 24컬럼**으로
  ...
  실제로는 구변일이라 `lib/dpf-column-map.ts`로 확인함). (참고: `DpfServiceRecordTable`(§8.2, 2026-09-13)은 이 판단과 달리
  기사처리/처리/완료 3줄 스택을 채택했다 — 두 표는 성격이 다르다. 여기(부착현황)는 차량 1행에 여러 파생값을 얹는 24컬럼짜리
  요약 행이라 스택이 행 높이를 흔들면 표 전체가 들쭉날쭉해지지만, 접수현황은 애초에 티켓 1건 = 1행이라 처리 단계별 날짜
  개수가 건마다 다른 게 자연스럽고(상세 카드의 `dates` 배열도 같은 이유로 필터링된 줄만 보여준다), 컬럼 수도 12개뿐이라
  들쭉날쭉함의 누적 효과가 훨씬 작다.)
