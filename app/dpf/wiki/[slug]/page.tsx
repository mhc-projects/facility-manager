...
import { ChevronRight, Edit, MessageSquare } from 'lucide-react';
...
export default function WikiSlugPage({ params }: { params: { slug: string } }) {
    ...
    return (
      <AdminLayout title="업무 지침" description="오류">
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 max-w-md">
          ...
          <Link href="/dpf/wiki" className="mt-2 inline-block text-blue-600 text-sm hover:underline">
            목차로 돌아가기
          </Link>
        </div>
      </AdminLayout>
  ...
  return (
    <AdminLayout title={node.title} description="DPF 업무 지침" actions={actions}>
      {/* 브레드크럼 */}
      <div className="flex items-center gap-1 text-sm text-gray-500 mb-4">
        <Link href="/dpf/wiki" className="hover:text-blue-600">업무 지침</Link>
