...
export default function FormDetailPage({ params }: { params: { code: string } }) {
  ...
  const formName = template?.name ?? formDef?.name ?? code;
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await fetch(`/api/wiki/forms/${code}/submit`, {
        method: 'POST',
  ...
  return (
    <AdminLayout title={formName} description="서식 작성" actions={actions}>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link href="/dpf/wiki/forms" className="flex items-center gap-1 hover:text-blue-600">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${vehicle ? 'lg:col-span-2' : 'lg:col-span-3'} order-1 lg:order-2`}>
          <div ref={printRef} className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-1 print:text-xl">{formName}</h2>
