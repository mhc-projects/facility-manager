...
import { CATEGORY_OPTIONS } from '@/components/dpf/ServiceRecordFormModal';
...
export default function DpfServicePage() {
  ...
  const search = useCallback(async (
    q: string, cat: string, st: string, gov: string, branch: string, billing: string,
    ...
    try {
      ...
      if (branch) params.set('service_branch', branch);
  ...
  const triggerSearch = useCallback((
    ...
    debounceRef.current = setTimeout(() => search(q, cat, st, gov, branch, billing, from, to, p), 300);
  }, [search]);

  useEffect(() => {
    triggerSearch(query, category, status, localGov, serviceBranch, billingStatus, dateFrom, dateTo, page);
  }, [query, category, status, localGov, serviceBranch, billingStatus, dateFrom, dateTo, page, triggerSearch]);
  ...
  useEffect(() => {
    fetch('/api/dpf/service-records/stats')
      ...
      .then(setStats)
      .catch(console.error);
  }, [result.total]); // 목록이 바뀔 때(등록/상태변경 등 외부 변화 반영 시점) 요약도 함께 갱신
  ...
  function clearAll() {
    ...
    setBillingStatus(''); setDateFrom(''); setDateTo(''); setPage(1);
  }

  const hasFilter = category || status || localGov || serviceBranch || billingStatus || dateFrom || dateTo;
  const activeFilterCount = [category, status, localGov, serviceBranch, billingStatus, dateFrom || dateTo]
