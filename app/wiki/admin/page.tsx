...
const DOMAIN_OPTIONS = [
  { value: 'dpf', label: 'DPF (운행차 배출가스 저감사업)', color: 'bg-blue-50 text-blue-700 border-blue-200' },
  { value: 'iot', label: 'IoT (방지시설 운영·모니터링)', color: 'bg-green-50 text-green-700 border-green-200' },
];
...
export default function WikiAdminPage() {
  ...
  const [domain, setDomain] = useState<'dpf' | 'iot'>('dpf');
  ...
  return (
    <AdminLayout title="업무지침 관리" description="지침서 업로드, AI 분석, Wiki 재인덱싱">
      <div className="space-y-6 max-w-3xl">
        <div>
          <Link href="/wiki" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-blue-600 transition-colors">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
          ...
          <div className="space-y-3">
            {/* 도메인 선택 */}
            <div>
              <label className="block text-sm text-gray-600 mb-1.5">지침서 유형</label>
              <div className="flex gap-2">
                {DOMAIN_OPTIONS.map(opt => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setDomain(opt.value as 'dpf' | 'iot')}
            ...
            <div>
              ...
              <input
                ...
                placeholder={domain === 'dpf' ? '예: 2026년 DPF 업무처리지침' : '예: 2026년 IoT 방지시설 운영지침'}
          ...
          <p className="mt-3 text-xs text-gray-500">
            업로드 후 AI가 도메인에 맞게 내용을 자동 분석합니다. 검토 후 적용 여부를 결정하세요.
          </p>
        </div>
