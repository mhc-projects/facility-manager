...
export async function DELETE(
  ...
  try {
    ...
    const { id } = params;
    // 자재 사용 이력, AS건의 출동단가 참조, 자재별 수동지정 매출단가(revenue_price_list_id) 참조 여부 확인
    const usageCheck = await pgQuery(
      `SELECT
        ...
        (SELECT COUNT(*) FROM as_material_usage WHERE revenue_price_list_id = $1) AS revenue_usage_count`,
      [id]
    );
    const usageCount =
      parseInt(usageCheck.rows[0]?.material_usage_count || '0') +
      parseInt(usageCheck.rows[0]?.dispatch_usage_count || '0') +
      parseInt(usageCheck.rows[0]?.revenue_usage_count || '0');
