...
// GET: 제조사 목록 조회 — 권한조회와 목록조회를 SQL 한 번으로 병합(직렬 DB 왕복 1회 절감)
export const GET = withApiHandler(async (request: NextRequest) => {
  ...
  const result = await queryOne(
    `SELECT
       ...
       (SELECT COALESCE(jsonb_agg(m ORDER BY m.sort_order, m.id), '[]'::jsonb) FROM (
          SELECT id, name, sort_order, is_active FROM manufacturers
        ) m) AS rows`,
    [userId]
  );
  ...
  return createSuccessResponse(result.rows ?? [], '제조사 목록을 조회했습니다.', 200, { noCache: true });
}, { logLevel: 'debug' });
