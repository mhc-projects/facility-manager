...
export async function POST(request: NextRequest) {
  try {
    ...
    const domain = (formData.get('domain') as string) || 'dpf';
    ...
    const { data: upload, error: dbError } = await supabaseAdmin
      ...
      .insert({ file_url: publicUrl, version_label: versionLabel, status: 'analyzing', domain })
      .select()
      .single();
    ...
    // 백그라운드 분석 (응답은 즉시 반환)
    analyzeGuidelineAsync(upload.id, buffer, versionLabel, domain).catch(console.error);
...
const ANALYSIS_PROMPTS: Record<string, string> = {
  dpf: `이 PDF는 운행차 배출가스 저감사업(DPF 매연저감장치) 업무처리지침입니다.
...
다음 형식으로 분석해주세요 (JSON만 반환, 마크다운 코드블록 없이):
{
  ...
  iot: `이 PDF는 환경설비 방지시설 IoT 모니터링 운영지침입니다.
직원들이 IoT 장비 운영, 데이터 관리, 이상 대응 실무에서 질문할 때 정확한 답변을 줄 수 있도록, 각 챕터의 내용을 원문에 충실하게 상세히 추출해주세요.
...
다음 형식으로 분석해주세요 (JSON만 반환, 마크다운 코드블록 없이):
{
  ...
  "chapters": [
    {
      "title": "챕터/섹션 제목 (원문 그대로)",
      "content": "해당 섹션의 내용을 아래 기준에 따라 상세히 서술 (최소 500자 이상):\n- 적용 대상 시설 및 장비 (Gateway, 센서, 측정기기 등)\n- 측정 항목 및 기준값 (구체적 수치 포함)\n- 점검 주기 및 절차 (일별/월별/연간)\n- 이상 감지 시 조치 절차 및 보고 체계\n- 데이터 보관 기간 및 전송 방식\n- 유지보수 기준 및 담당자 역할\n- 기타 중요 규정 및 예외사항\n위 항목 중 해당 챕터에 포함된 내용만 서술하되, 원문의 구체적인 수치·조건·절차를 빠짐없이 포함하세요.",
      "key_points": ["구체적 핵심 내용 1 (수치/기준 포함)", "핵심 내용 2", "핵심 내용 3"]
    }
  ],
  "form_changes": [{"form_name": "서식명", "change_type": "추가/수정/삭제", "description": "변경 내용"}]
}`,
...
async function analyzeGuidelineAsync(uploadId: string, buffer: ArrayBuffer, versionLabel: string, domain = 'dpf') {
  ...
  try {
    ...
    const analysisPrompt = ANALYSIS_PROMPTS[domain] ?? ANALYSIS_PROMPTS.dpf;
    const result = await model.generateContent([pdfPart, analysisPrompt]);
    ...
    if (chapters.length > 0) {
      await createWikiNodes(chapters, versionLabel, domain);
    }
...
async function createWikiNodes(chapters: ChapterAnalysis[], versionLabel: string, domain = 'dpf') {
  const rootSlug = `guideline-${versionLabel.replace(/[^a-zA-Z0-9가-힣]/g, '-').replace(/-+/g, '-')}`;
  const domainLabel = domain === 'iot' ? 'IoT 방지시설 운영지침' : 'DPF 업무처리지침';
